...
async def list_professors(db: AsyncSession) -> list[AdminProfessorOut]:
    ...
    return [
        AdminProfessorOut(
            id=p.id, name=p.name, initials=_professor_initials(p.name),
            review_count=0, updated_at=p.updated_at,
        )
        for p in profs
    ]
...
async def create_professor(
    ...
    p = await admin_repo.create_professor(db, name=payload.name)
...
async def update_professor(
    ...
    p = await admin_repo.update_professor(db, professor_id, name=payload.name)
    if p is None:
        raise HTTPException(
